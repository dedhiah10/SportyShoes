...
public class ProductController {
	...
	public ModelAndView index(HttpServletRequest req, HttpServletResponse resp) {
		if(!flag) {prods = repopro.findAll(); flag = true;}
	...
	@RequestMapping("/LoginController") @GetMapping("/LoginController")
	public String login(HttpServletRequest req) {
		HttpSession session=req.getSession(false);
		if(session != null && session.getAttribute("uname") != null) { return "redirect:/";}
	...
	public String checker(HttpServletRequest req, HttpServletResponse resp) {
		...
		for(User usr: usrlist) {
			if(usr.getName().equals(req.getParameter("pword"))) {
				...
				} else {session.setAttribute("adminRole", false); session.setAttribute("admin", false);}
				flag = true;
	...
	@RequestMapping("/NewUser") @GetMapping("/NewUser")
	...
	public String userAdd(User user, HttpServletRequest req, HttpServletResponse resp) {
		...
		if(req.getParameter("roleID") != null) {
			if(req.getParameter("roleID").equals("Truly")) {user.setRole(true);};
		}
	...
	boolean update(User user) {
		...
		/*Iterator<User> itr = usrlist.listIterator();
		while(itr.hasNext()) {
			User usr = itr.next();
			if(
				usr.getCountry().equalsIgnoreCase(user.getCountry()) && 
				usr.getName().equalsIgnoreCase(user.getName()) &&
				usr.getDateOfBirth().equals(user.getDateOfBirth())
			) {exists = true; System.out.println("Entered");}
		}*/
		for(User usr: usrlist) {
			if(
				usr.getCountry().equalsIgnoreCase(user.getCountry()) && 
		...
		if(exists == false) {repousr.save(user);}	
		//System.out.println(System.currentTimeMillis() - time);
		return exists;
	}
	
	@GetMapping("/SignOut") @RequestMapping("/SignOut")
	public String signout(HttpServletRequest req) {
		req.getSession().invalidate();
		return "redirect:/";
	}
	
	@RequestMapping("/CartAdder") @GetMapping("/CartAdder")
	public String cartadd(HttpServletRequest req) {
		HttpSession session=req.getSession(false);
		if(session != null && session.getAttribute("uname") != null) {
			usrprod.replace((Integer) session.getAttribute("uID"), (usrprod.get(session.getAttribute("uID"))+"_"+req.getParameter("pID")));
			//System.out.println((usrprod.get(session.getAttribute("uID"))));
			return "redirect:/";
		} else {return "redirect:/LoginController";}
	}
	
	@RequestMapping("/CartManager") @GetMapping("/CartManager")
	...
	@RequestMapping("/CartMaker") @GetMapping("/CartMaker")
	...
	@GetMapping("/BuyAdder") @RequestMapping("/BuyAdder")
	public String buyprod(HttpServletRequest req) {
		...
		if(session != null && session.getAttribute("uname") != null) {
			...
			x.add(new ProdTime(Integer.parseInt(req.getParameter("pID")), new Date(System.currentTimeMillis()), new Time(System.currentTimeMillis())));
	...
	@GetMapping("/AccountDetails") @RequestMapping("/AccountDetails")
	...
	@GetMapping("/AdminRights") @RequestMapping("/AdminRights")
